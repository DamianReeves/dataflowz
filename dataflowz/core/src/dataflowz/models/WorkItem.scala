 ...
 */
final case class WorkItem[+Data](data:Data, status:WorkItemStatus, history:List[HistoryEntry]) { self =>
  ...
  def exclude[D  >: Data : Tag](reason:String, message:String):WorkItem[D] =
    copy(
      ...
      history = nextHistoryEntry[D](WorkItemStatus.Excluded, reason = reason, message = message) :: self.history
    )
  ...
  def fail[D >: Data : Tag](reason:String, message:String): WorkItem[D] =
    copy(
      ...
      history = nextHistoryEntry[D](WorkItemStatus.Failed, reason = reason, message = message) :: self.history
    )
  ...
  def recover[D >: Data : Tag](reason:String, message:String):WorkItem[D] =
    copy(
      ...
      history = nextHistoryEntry[D](WorkItemStatus.Ok, reason = reason, message = message) :: self.history
    )
  ...
  def skip[D >: Data : Tag](reason:String, message:String): WorkItem[D] =
    copy(
      ...
      history = nextHistoryEntry[D](WorkItemStatus.Skipped, reason, message) :: self.history
    )

  def whenOk[D >: Data: Tag](action: WorkItem[D] => Unit)(onError: (self.type, Throwable) =>  (WorkItemStatus,String, String)): WorkItem[D] =
    try {
      action(self)
      self
    }catch {
  ...
  private def nextHistoryEntry[D >: Data](status:WorkItemStatus, reason:String, message:String)
                              (implicit evTag:Tag[D],  sequenceProvider: WorkItemSequenceProvider[D]):WorkItem.HistoryEntry =
...
object WorkItem {
  type WorkItemSequenceProvider[Data] = SequenceProvider.Aux[WorkItem[Data], Long]

  implicit def workItemSequenceProviding[A]: Aux[WorkItem[A], Long] = SequenceProvider.longSequenceProvider { workItem:WorkItem[A] =>
  ...
  //       the `WorkItem` type.
  final case class HistoryEntry private (status:WorkItemStatus, reason:String, dataType:String, message:String, sequenceNumber:Long)
  object HistoryEntry {
    private[WorkItem] def initialFor[Data](message:Option[String])(implicit tagged:Tag[Data]): HistoryEntry =
      HistoryEntry(
        status = WorkItemStatus.Ok,
        reason = "Created",
