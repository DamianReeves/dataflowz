...
sealed trait FlowStep[-S1, +S2, +A] { self =>
  ...
  final def flatten[S3, B](
      implicit ev: A <:< FlowStep[S2, S3, B]
  ): FlowStep[S1, S3, B] =
    flatMap(ev)
  ...
  final def transformM[S3, B](
      f: (S2, A) => FlowStep[S2, S3, B]
  ): FlowStep[S1, S3, B] =
    (self <*> get).flatMap { case (a, s) => f(s, a) }
    ...
    def loop(self: FlowStep[Any, Any, Any])(s: Any): (Any, Any) =
      self match {
        ...
        case FlatMap(Modify(f), continue) =>
          f(s) match { case (s, a) => loop(continue(a))(s) }
        case FlatMap(FlatMap(x, f), g) =>
          loop(x.flatMap(a => f(a).flatMap(g)))(s)
      }
  ...
  final def zipWith[S3, B, C](
      that: FlowStep[S2, S3, B]
  )(f: (A, B) => C): FlowStep[S1, S3, C] =
...
object FlowStep {
  ...
  private[dataflowz] final case class Succeed[S, +A](value: A)
      extends FlowStep[S, S, A]
  private[dataflowz] final case class Modify[-S1, +S2, +A](run: S1 => (S2, A))
      extends FlowStep[S1, S2, A]
  private[dataflowz] final case class FlatMap[-S1, S2, +S3, A, +B](
      value: FlowStep[S1, S2, A],
      continue: A => FlowStep[S2, S3, B]
